...
uint64 sys_symlink()
{
  ...
  // begin_op() 和 end_op()防止多线程或多进程环境下的竞态条件，确保在操作期间不会中断。
  begin_op();
  ip = create(path, T_SYMLINK, 0, 0); // 创建一个文件，返回其 inode
  if (ip == 0)
  {
    end_op();
    return -1;
  }
  // writei 往某个 inode 中写数据，这里把软链接想要指向的路径写进去了
  if (writei(ip, 0,(uint64) tar_path, 0, strlen(tar_path)) < 0)
  {
    end_op();
    return -1;
  }
