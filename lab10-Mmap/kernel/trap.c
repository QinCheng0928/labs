...
void usertrap(void)
{
  ...
  {
    uint64 va = r_stval();
    // p->sz 通常表示堆的末尾（即分配的最高虚拟地址）。
    // MAXVA 是虚拟地址空间的上限，表示用户态程序可以访问的最大虚拟地址。
    // PGROUNDUP(va) 将 va 向上取整到最近的页边界。
    // PGROUNDDOWN(p->trapframe->sp) 将进程栈指针向下取整到最近的页边界。
    // va在进程的地址空间范围内。小于允许的最大虚拟地址。不能位于进程栈的页内，以避免内存分配与栈冲突。
