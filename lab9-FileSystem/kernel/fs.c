...
// 目的是在文件系统中找到一个逻辑块号对应的物理块号
// 如果对应的物理块不存在，它会分配一个新的物理块
// bn ：逻辑块号（block number）
static uint
bmap(struct inode *ip, uint bn)
{
  // a：指向一个整数数组的指针，用于处理间接块。
  uint addr, *a;
  // bp：指向一个缓冲区结构体，用于读取块数据。
  struct buf *bp, *bp2;
  ...
  if (bn < NBI_INDIRECT)
  {
    ...
    if ((addr = a[idx_b1]) == 0)
    { // 一个一级块负责 256 个二级块，这里检测对应一级块是否存在
